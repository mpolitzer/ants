...
void move(int index, char dir, struct game_state* Game, struct game_info* Info) {
	...
	int *row = &Game->my_ants[index].row;
	int *col = &Game->my_ants[index].col;
	char *val = &Info->map[mkindex(*row, *col, Info)];
	...
	case 'N':
		if (*row != 0) *row -= 1;
		else *row = Info->rows - 1;
		break;
	case 'E':
		if (*col != Info->cols - 1) *col += 1;
		else *col = 0;
		break;
	case 'S':
		if (*row != Info->rows - 1) *row += 1;
		else *row = 0;
		break;
	case 'W':
		if (*col != 0) *col -= 1;
		else *col = Info->cols - 1;
		break;
	}
	val = &Info->map[mkindex(*row, *col, Info)];
